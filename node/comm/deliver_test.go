...
import (
	...
	"github.com/hyperledger/fabric-x-orderer/node/comm/mocks"
...
func (d *countingDialer) Dial(address comm.EndpointCriteria) (*grpc.ClientConn, error) {
...
func (ds *deliverServer) endpointCriteria() comm.EndpointCriteria {
	return comm.EndpointCriteria{Endpoint: ds.srv.Address()}
}
...
func newBlockPuller(dialer comm.Dialer, orderers ...string) *comm.BlockPuller {
	return &comm.BlockPuller{
		Dialer:              dialer,
		Channel:             "mychannel",
...
func endpointCriteriaFromEndpoints(orderers ...string) []comm.EndpointCriteria {
	var res []comm.EndpointCriteria
	for _, orderer := range orderers {
		res = append(res, comm.EndpointCriteria{Endpoint: orderer})
	}
	return res
}
...
func TestBlockPullerFailures(t *testing.T) {
	...
	failStream := func(osn *deliverServer, _ *comm.BlockPuller) {
	...
	malformBlockSignatureAndRecreateOSNBuffer := func(osn *deliverServer, bp *comm.BlockPuller) {
	...
	noFailFunc := func(_ *deliverServer, _ *comm.BlockPuller) {}

	recover := func(osn *deliverServer, bp *comm.BlockPuller) func(entry zapcore.Entry) error {
	...
	for _, testCase := range []struct {
		name       string
		logTrigger string
		beforeFunc func(*deliverServer, *comm.BlockPuller)
		failFunc   func(*deliverServer, *comm.BlockPuller)
	}{
		{
			name:       "failure at probe",
			logTrigger: "skip this for this test case",
			beforeFunc: func(osn *deliverServer, bp *comm.BlockPuller) {
				failStream(osn, nil)
...
func TestImpatientStreamDetectSelf(t *testing.T) {
	...
	newStream := comm.NewImpatientStream(conn, time.Millisecond*100)
