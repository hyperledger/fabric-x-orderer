...
import (
	...
	node_crypto "github.com/hyperledger/fabric-x-orderer/node/crypto"
...
// CertBytes implements tlsgen.CA.
func (c dummyCA) CertBytes() []byte {
	return c.Cert
}

// NewClientCertKeyPair implements tlsgen.CA.
func (c dummyCA) NewClientCertKeyPair() (*tlsgen.CertKeyPair, error) {
	panic("unimplemented")
}

// NewIntermediateCA implements tlsgen.CA.
func (c dummyCA) NewIntermediateCA() (tlsgen.CA, error) {
	panic("unimplemented")
}

// NewServerCertKeyPair implements tlsgen.CA.
func (c dummyCA) NewServerCertKeyPair(hosts ...string) (*tlsgen.CertKeyPair, error) {
	return &tlsgen.CertKeyPair{Cert: c.Cert, Key: c.Key}, nil
}

// Signer implements tlsgen.CA.
func (c dummyCA) Signer() crypto.Signer {
	panic("unimplemented")
}
...
func createRouterTestSetupFromConfig(t *testing.T, dir string, partyID types.PartyID, numOfShards int) *routerTestSetup {
	...
	for i := range numOfShards {
		...
		ca := dummyCA{Cert: batcher_config.TLSCertificateFile, Key: batcher_config.TLSPrivateKeyFile}

		batcher := NewStubBatcher(t, dummyCA{Cert: batcher_config.TLSCertificateFile, Key: batcher_config.TLSPrivateKeyFile}, partyID, types.ShardID(i+1))
		batchers = append(batchers, &batcher)

		shards = append(shards, node_config.ShardInfo{ShardId: types.ShardID(i + 1), Batchers: []node_config.BatcherInfo{{PartyID: 1, Endpoint: batcher.server.Address(), TLSCACerts: []node_config.RawBytes{ca.CertBytes()}}}})
	}
...
// TestClientRouterBrodcastSignedRequests verifies that the router correctly accepts and
...
func TestClientRouterBrodcastSignedRequests(t *testing.T) {
	// t.Skip()
	...
	res := submitSignedBroadcastRequests(t, org, keyBytes, certBytes, testSetup.clientConn, 10)
...
func submitSignedBroadcastRequests(t *testing.T, org string, keyBytes []byte, certBytes []byte, conn *grpc.ClientConn, numOfRequests int) (res testStreamResult) {
	...
	go func() {
		...
		for j := 0; j < numOfRequests; j++ {
			...
			env := tx.CreateSignedStructuredEnvelope(payload, (*node_crypto.ECDSASigner)(pk), certBytes, org)
			err := stream.Send(env)
			if err != nil {
				return
			}
