...
func TestBatcherDeliverServiceServesOnlyConfiguredNodes(t *testing.T) {
	...
	t.Run("the batcher of the same shard in another party", func(t *testing.T) {
		response, err := pullBatch(t, endpoint, channelName, sameShardSigner)
		require.NoError(t, err)
		require.NotNil(t, response.GetBlock())
	})

	t.Run("a batcher of another shard", func(t *testing.T) {
		response, err := pullBatch(t, endpoint, channelName, otherShardSigner)
	...
	t.Run("the certificate of the assembler, signed by another key", func(t *testing.T) {
		response, err := pullBatch(t, endpoint, channelName, &mismatchedSigner{cert: assemblerCert, key: foreignSigner})
		require.ErrorIs(t, err, io.EOF)
		require.Nil(t, response.GetBlock())
	})
}
...
func serveBatcherDeliverService(t *testing.T, shardID types.ShardID, partyID types.PartyID, channelID string, accessControl *deliver.NodeVerifier) string {
