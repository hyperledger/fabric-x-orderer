...
func TestRequestsInspectAndVerify(t *testing.T) {
	...
	require.NoError(t, err)
	largeReq, err := proto.Marshal(tx.CreateStructuredRequest(make([]byte, 102))) // large request, mapped to shard 1
	require.NoError(t, err)
	...
	t.Run("verify empty request", func(t *testing.T) {
		...
		emptyPayloadReq2, err := proto.Marshal(&protos.Request{Payload: []byte{}})
		require.NoError(t, err)
		require.ErrorContains(t, verifier.VerifyRequest(emptyPayloadReq2), "empty")
	})
	...
	t.Run("verify request with sig filter", func(t *testing.T) {
		badReq, err := proto.Marshal(&protos.Request{Payload: []byte("1234")}) // should map to shard 1
		require.NoError(t, err)
		require.ErrorContains(t, verifier.VerifyRequest(badReq), "failed to convert request to signedData")
	})
