...
func createBatcherRulesVerifier(config *config.BatcherNodeConfig) *requestfilter.RulesVerifier {
	...
	// rv.AddRule(requestfilter.NewSigFilter(config, policies.ChannelWriters)) - TBD
	return rv
}
...
func (r *RequestsInspectorVerifier) VerifyRequestFromRouter(req *comm.Request) error {
	// check that the shard is correct
	if err := r.VerifyRequestShard(req); err != nil {
		rawReq, _ := proto.Marshal(req)
		return errors.Errorf("failed verifying request from router with id: %s; err: %v", r.RequestID(rawReq), err)
	}

	// TBD - check config sequence, and verify again if needed.
	err := r.requestVerifier.Verify(req)
	if err != nil {
		rawReq, _ := proto.Marshal(req)
		return errors.Errorf("failed verifying request from router with id: %s; err: %v", r.RequestID(rawReq), err)
	}
...
func (r *RequestsInspectorVerifier) VerifyRequest(req []byte) error {
	...
	err = r.requestVerifier.Verify(&request)
	if err != nil {
		return errors.Errorf("failed verifying request with id: %s; err: %v", r.RequestID(req), err)
	}
	return nil
