...
func fillHeapWithBatches(t *testing.T, heap *assembler.BatchHeap[int], shardId types.ShardID, party types.PartyID, numberOfBatches int) []*assembler.BatchHeapItem[int] {
	items := []*assembler.BatchHeapItem[int]{}
	for i := 0; i < numberOfBatches; i++ {
		item := &assembler.BatchHeapItem[int]{
			Batch: testutil.CreateEmptyMockBatch(shardId, party, types.BatchSequence(i), []byte{byte(i)}),
			Value: i,
		}
...
func testHeapWrongShardOrParty(t *testing.T, heapOp func(*assembler.BatchHeap[int], types.BatchID)) {
	t.Run("WrongShardParameterShouldPanic", func(t *testing.T) {
		// Arrange
		heap := assembler.NewBatchHeap(1, 1, sequenceComparator)
		fillHeapWithBatches(t, heap, 1, 1, 3)
...
func TestHeap_NonPrimitiveGenericValueWorksAsExpected(t *testing.T) {
	// Arrange
	heap := assembler.NewBatchHeap(1, 1, func(item1, item2 *assembler.BatchHeapItem[[]byte]) bool { return item1.Value[0] < item2.Value[0] })
	...
	for i := byte(0); i < 10; i++ {
		item := &assembler.BatchHeapItem[[]byte]{
			Batch: testutil.CreateEmptyMockBatch(1, 1, types.BatchSequence(i), []byte{i}),
			Value: []byte{i, i, i},
		}
		items = append(items, item)
		require.NoError(t, heap.Push(item))
...
func TestHeap_Peek(t *testing.T) {
	...
	t.Run("ReturnsMinBatch", func(t *testing.T) {
		// Arrange
		heap := assembler.NewBatchHeap(1, 1, sequenceComparator)
		heapItems := fillHeapWithBatches(t, heap, 1, 1, 3)
...
func TestHeap_Pop(t *testing.T) {
	...
	t.Run("MultiplePopReturnBatchesByOrder", func(t *testing.T) {
		// Arrange
		heap := assembler.NewBatchHeap(1, 1, sequenceComparator)
		heapItems := fillHeapWithBatches(t, heap, 1, 1, 10)
