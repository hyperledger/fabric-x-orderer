...
func TestConsensus(t *testing.T) {
	...
	assert.NoError(t, err)
	baf123id2p1s1, err := batcher.CreateBAF(crypto.ECDSASigner(*sk2), 2, 1, dig123, 1, 1, 0, 0, baf123id1p1s1.Signature())
	assert.NoError(t, err)
	baf123id3p1s1, err := batcher.CreateBAF(crypto.ECDSASigner(*sk3), 3, 1, dig123, 1, 1, 0, 0, baf123id1p1s1.Signature())
	assert.NoError(t, err)
	baf123id4p1s1, err := batcher.CreateBAF(crypto.ECDSASigner(*sk4), 4, 1, dig123, 1, 1, 0, 0, baf123id1p1s1.Signature())
	assert.NoError(t, err)

	dig124 := append([]byte{1, 2, 4}, dig...)
	baf124id2p2s1, err := batcher.CreateBAF(crypto.ECDSASigner(*sk2), 2, 2, dig124, 2, 1, 0, 0, nil)
	assert.NoError(t, err)
	baf124id1p2s1, err := batcher.CreateBAF(crypto.ECDSASigner(*sk1), 1, 2, dig124, 2, 1, 0, 0, baf124id2p2s1.Signature())
	assert.NoError(t, err)
...
func TestAssembleProposalAndVerify(t *testing.T) {
	...
	assert.NoError(t, err)
	baf123id2p1s1, err := batcher.CreateBAF(crypto.ECDSASigner(*sks[1]), 2, 1, dig123, 1, 1, 0, 0, baf123id1p1s1.Signature())
	assert.NoError(t, err)
	baf123id1p1s1cs1, err := batcher.CreateBAF(crypto.ECDSASigner(*sks[0]), 1, 1, dig123, 1, 1, 1, 0, baf123id1p1s1.Signature())
	assert.NoError(t, err)
	baf123id2p1s1cs1, err := batcher.CreateBAF(crypto.ECDSASigner(*sks[1]), 2, 1, dig123, 1, 1, 1, 0, baf123id1p1s1.Signature())
	assert.NoError(t, err)

	dig124 := append([]byte{1, 2, 4}, dig...)
	baf124id1p1s2, err := batcher.CreateBAF(crypto.ECDSASigner(*sks[0]), 1, 1, dig124, 1, 2, 0, 0, nil)
	assert.NoError(t, err)
	baf124id3p1s2, err := batcher.CreateBAF(crypto.ECDSASigner(*sks[2]), 3, 1, dig124, 1, 2, 0, 0, baf124id1p1s2.Signature())
	assert.NoError(t, err)
...
func TestVerifyProposal(t *testing.T) {
	...
	assert.NoError(t, err)
	baf123id2p1s1, err := batcher.CreateBAF(crypto.ECDSASigner(*sks[1]), 2, 1, dig123, 1, 1, 0, 1, baf123id1p1s1.Signature())
	assert.NoError(t, err)
