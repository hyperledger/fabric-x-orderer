...
func (c *Complaint) FromBytes(bytes []byte) error {
	...
	sigSize := int(binary.BigEndian.Uint16(bytes[20:22]))
	sigEnd := 22 + sigSize
	if sigEnd+2 > len(bytes) {
		return fmt.Errorf("input too small for signature (%d < %d)", len(bytes), sigEnd+2)
	}
	c.Signature = append([]byte(nil), bytes[22:sigEnd]...)
	rSize := int(binary.BigEndian.Uint16(bytes[sigEnd : sigEnd+2]))
	reasonEnd := sigEnd + 2 + rSize
	if reasonEnd > len(bytes) {
		return fmt.Errorf("input too small for reason (%d < %d)", len(bytes), reasonEnd)
	}
	c.Reason = string(bytes[sigEnd+2 : reasonEnd])
	return nil
}
...
func (c *Complaint) fromProto(pc *stateprotos.Complaint) error {
	if pc.GetShard() > math.MaxUint16 {
		return fmt.Errorf("the Complaint Shard value %d that exceeds uint16 maximum %d", pc.Shard, math.MaxUint16)
	}
