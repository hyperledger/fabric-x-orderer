...
func (c *BroadcastTxClient) createSendStreams() error {
	...
	for i := 0; i < len(userConfig.RouterEndpoints); i++ {
		...
		if !ok {
			...
			if stream != nil {
				signer, certBytes, err := c.buildCryptoMaterials(types.PartyID(i + 1))
				if err != nil {
			...
			if streamInfo.stream == nil {
				...
				if stream != nil {
					...
					signer, certBytes, err := c.buildCryptoMaterials(streamInfo.partyID)
					if err != nil {
						return fmt.Errorf("failed to build crypto materials: %v", err)
					}
...
func (c *BroadcastTxClient) buildCryptoMaterials(partyID types.PartyID) (*crypto.ECDSASigner, []byte, error) {
	keyBytes, err := os.ReadFile(filepath.Join(c.configDir, fmt.Sprintf("crypto/ordererOrganizations/org%d/users/user/msp/keystore/priv_sk", partyID)))
	if err != nil {
	...
	certBytes, err := os.ReadFile(filepath.Join(c.configDir, fmt.Sprintf("crypto/ordererOrganizations/org%d/users/user/msp/signcerts/sign-cert.pem", partyID)))
	if err != nil {
