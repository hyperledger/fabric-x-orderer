...
// TestUpdatePartyBatchingParameters verifies that updating a party's batching parameters via a config update succeeds,
// and that the party can continue processing transactions after the config update with the new batching parameters.
func TestUpdatePartyBatchingParameters(t *testing.T) {
	// Prepare Arma config and crypto and get the genesis block
	dir, err := os.MkdirTemp("", t.Name())
	require.NoError(t, err)
	defer os.RemoveAll(dir)
	...
	// Pull blocks to verify all transactions are included
	userBlockHandler := verifyMaxMessageCount{MaxMessageCountPerBlock: maxMessageCount}
	PullFromAssemblers(t, &BlockPullerOptions{
		UserConfig:   userConfig,
		Parties:      parties,
		Transactions: totalTxNumber + 1, // including config update tx
...
type verifyMaxMessageCount struct {
	MaxMessageCountPerBlock int
	lock                    sync.Mutex
}

func (v *verifyMaxMessageCount) HandleBlock(t *testing.T, block *common.Block) error {
	v.lock.Lock()
	defer v.lock.Unlock()
	...
	if isGenesisBlock || isConfigBlock {
		return nil
	}

	if v.MaxMessageCountPerBlock < len(block.GetData().GetData()) {
		return fmt.Errorf("block contains %d transactions, which exceeds the max message count per block of %d", len(block.GetData().GetData()), v.MaxMessageCountPerBlock)
	}
	return nil
}
