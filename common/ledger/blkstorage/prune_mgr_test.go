...
//  4. For blocks 0, 1 and 19, expect retrieveBlockByNumber, retrieveBlockHeaderByNumber and
//     retrieveBlocks to fail with ErrPruned.
...
// 5. Expect reopening to fail with an error naming the missing block file and the recorded bound.
func TestOpenPrunedLedgerWithStalePruneInfo(t *testing.T) {
	...
	require.ErrorContains(t, err, "blocks readable from [10]")
}
...
// 5. Expect reopening to fail naming the missing block file and the bound a record-less store reports.
func TestOpenPrunedLedgerWithLostPruneInfo(t *testing.T) {
	...
	defer reopened.Cleanup()
	_, err := reopened.provider.Open("testLedger")
	require.ErrorContains(t, err, "block file [0] is missing")
	require.ErrorContains(t, err, "blocks readable from [0]")
}

// A prune whose point falls inside the lowest surviving file moves the bound and leaves the first stored
// file at 0, so a first stored file of 0 is an ordinary record and not the absence of one.
//
// Scenario:
// 1. Store 50 blocks across 5 block files.
// 2. Write prune info at block 5, inside block file 0, which stays the first stored file.
// 3. Rewind the index savepoint to block 45 and close the store.
// 4. Delete block file 0, so the prune info and the files disagree.
// 5. Expect reopening to fail reporting the recorded bound rather than treating the record as absent.
func TestOpenPrunedLedgerWithBoundInsideTheLowestFile(t *testing.T) {
	path := t.TempDir()

	env, w, blocks := newPrunableTestLedger(t, path, 50)
	rootDir := w.blockfileMgr.rootDir
	require.NoError(t, w.blockfileMgr.pruner.setInfo(&pruneInfo{
		firstReadableBlockNum:   5,
		firstStoredBlockfileNum: 0,
	}))
	rewindIndexSavepoint(t, w.blockfileMgr, uint64(len(blocks)-5))
	env.provider.Close()
	require.NoError(t, os.Remove(deriveBlockfilePath(rootDir, 0)))

	reopened := newTestEnv(t, NewConf(path, 0))
	defer reopened.Cleanup()
	_, err := reopened.provider.Open("testLedger")
	require.ErrorContains(t, err, "block file [0] is missing")
	require.ErrorContains(t, err, "blocks readable from [5]")
}
