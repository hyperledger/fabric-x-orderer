...
// file below it, producing the on-disk state that reclamation leaves behind.
func pruneFilesUpTo(t *testing.T, mgr *blockfileMgr, firstStoredBlockfileNum int) {
	// Reclamation never removes the file holding lastPersistedBlock, so neither may this fixture.
	require.LessOrEqual(t, uint64(firstStoredBlockfileNum)*blocksPerFileForTest, mgr.blockfilesInfo.lastPersistedBlock,
		"fixture would reclaim the file holding lastPersistedBlock")
...
// file still on disk. Reversed, the read guard would admit a block whose file has been reclaimed.
...
//  3. Expect the readable bound to be 5 with no file reclaimed and every block file still present.
//  4. Expect blocks 0..4 to fail with ErrPruned and blocks 5..49 to be returned.
func TestPruneBeforeAdvancesBoundWithoutReclaiming(t *testing.T) {
	env, w, blocks := newPrunableTestLedger(t, t.TempDir(), 50)
	defer env.Cleanup()
	mgr := w.blockfileMgr
...
//  3. Expect the bound to move to 5 with no file reclaimed.
...
//  3. Expect no error, the marker to stay 0, and nothing to be reclaimed.
...
//  2. Call pruneBefore(MaxUint64).
//  3. Expect no file to be reclaimed, because the only file holds the last block, and the bound to be
//     capped at block 4.
...
//  3. Expect only block file 0 to be reclaimed and the marker to be block 10.
//  4. Call pruneBefore(30), exactly the first block of block file 3.
//  5. Expect block files 1 and 2 to be reclaimed and the marker to be block 30.
//  6. Call pruneBefore(40) and expect block file 3 to be reclaimed, leaving only block file 4.
...
//  4. Expect block file 4 to be reclaimed now that it no longer holds the last block, leaving only block
...
//  4. Expect an error, and the two files reclaimed before the failure to stay reclaimed: block files 0
...
//  3. Call pruneBefore(20), which has nothing new to reclaim.
...
//  4. Expect an error naming that block file, and nothing to have been reclaimed: the probe for the first
...
//  2. Expect a block hash lookup for a reclaimed block to fail, but not with ErrPruned: its hash index
//     entry went with the file, and that path has no availability guard.
//  3. Expect a transaction lookup for a reclaimed block to fail the same way.
...
//  2. Call pruneBefore(MaxUint64).
//  3. Expect nothing to change: no file is reclaimable and the bound cannot pass the last block.
...
//     the file whose last block is the bound keeps its blocks, since reclamation works whole files.
