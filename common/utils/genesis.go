...
func EmptyGenesisBlock(channelID string) *common.Block {
	...
	ordererBlockMetadata, err := ledger.AssemblerBlockMetadataToBytes(state.NewAvailableBatch(0, types.ShardIDConsensus, 0, genesisDigest), &state.OrderingInformation{DecisionNum: 0, BatchCount: 1, BatchIndex: 0}, 1)
	if err != nil {
