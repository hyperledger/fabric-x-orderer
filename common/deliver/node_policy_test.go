...
func TestNewNodeVerifier(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "a node that connects to the service and one that does not, sharing a certificate",
			parties: []*ordererpb.PartyConfig{{
				PartyID:         1,
				ConsenterConfig: &ordererpb.ConsenterNodeConfig{SignCert: certPEM},
				AssemblerConfig: &ordererpb.AssemblerNodeConfig{SignCert: certPEM},
			}},
			roles: []deliver.NodeRole{deliver.RoleBatcher, deliver.RoleAssembler},
			error: "the consenter of party 1 and the assembler of party 1 share a signing certificate",
		},
		{
			name: "a signing certificate that cannot be parsed",
			parties: []*ordererpb.PartyConfig{{
		...
		},
		{
			...
			parties: []*ordererpb.PartyConfig{{
				PartyID:      1,
				RouterConfig: &ordererpb.RouterNodeConfig{SignCert: signCertOfCurve(t, elliptic.P384())},
			}},
			roles: allRoles,
...
// Scenario:
//  1. Place one signing certificate in the shared configuration as both the router and the
//     consenter of party 1, and a second one as its assembler.
//  2. Build a verifier for a service that only assemblers connect to.
//  3. Expect the verifier to be built, since neither the router nor the consenter connects to it.
//  4. Check the policy on a request the assembler signed, and expect it to be authorized.
func TestNewNodeVerifierOfCertificateSharedByRolesThatDoNotConnect(t *testing.T) {
	sharedPEM, _ := generateSignCert(t)
	assemblerPEM, assemblerKey := generateSignCert(t)

	parties := []*ordererpb.PartyConfig{{
		PartyID:         1,
		RouterConfig:    &ordererpb.RouterNodeConfig{SignCert: sharedPEM},
		ConsenterConfig: &ordererpb.ConsenterNodeConfig{SignCert: sharedPEM},
		AssemblerConfig: &ordererpb.AssemblerNodeConfig{SignCert: assemblerPEM},
	}}

	verifier, err := deliver.NewNodeVerifier(bundleOf(t, parties), deliver.RoleAssembler)
	require.NoError(t, err)

	signer := &testSigner{certPEM: assemblerPEM, key: assemblerKey}
	require.NoError(t, verifier.CheckPolicy(signRequest(t, signer), "arma"))
}
