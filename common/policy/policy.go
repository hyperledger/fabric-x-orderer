...
func BuildVerifiedConfigRequest(request *protos.Request, configEnvelope *cb.ConfigEnvelope, bundle channelconfig.Resources, signer identity.SignerSerializer, verifier *requestfilter.RulesVerifier) (*protos.Request, error) {
	...
	config, err := createSignedConfigEnvelopeWithTxID(bundle.ConfigtxValidator().ChannelID(), signer, configEnvelope)
	if err != nil {
...
// createSignedConfigEnvelopeWithTxID wraps the config envelope in an outer HeaderType_CONFIG
...
func createSignedConfigEnvelopeWithTxID(channelID string, signer identity.SignerSerializer, configEnvelope *cb.ConfigEnvelope) (*cb.Envelope, error) {
	if signer == nil {
