...
//go:generate counterfeiter -o mocks/configrulesverifier.go . ConfigRolesVerifier
type ConfigRolesVerifier interface {
	ValidateNewConfig(envelope *common.Envelope) error
}

type DefaultConfigRolesVerifier struct{}

// ValidateNewConfig validates the rules of the new configuration is valid before it is applied.
func (cr *DefaultConfigRolesVerifier) ValidateNewConfig(envelope *common.Envelope) error {
