...
func UnmarshalBlockMetadataSignatures(bytes [][]byte) (*common.Metadata, error) {
	if len(bytes) <= int(common.BlockMetadataIndex_SIGNATURES) {
		return nil, errors.Errorf("no signatures in block metadata")
	}
	md := &common.Metadata{}
	if err := proto.Unmarshal(bytes[common.BlockMetadataIndex_SIGNATURES], md); err != nil {
		return nil, errors.Wrapf(err, "error unmarshalling signatures from metadata: %v", err)
	}
	return md, nil
}
