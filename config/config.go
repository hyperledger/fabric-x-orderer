...
func readBootstrapConfigBlock(conf *LocalConfig) (*common.Block, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("could not read bootstrap block from %s: %w", blockPath, err)
	}
	bootstrapBlock, err := protoutil.UnmarshalBlock(data)
	if err != nil {
		return nil, fmt.Errorf("error unmarshalling bootstrap block from %s: %w", blockPath, err)
	}
