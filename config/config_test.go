...
// TestExtractedBundleAuthorizesInternalDeliver checks that the configuration bundle a node extracts
// carries the shared configuration its access control is built from, and that this access control
// resolves the signing identities of a network generated by armageddon.
//
// Scenario:
//  1. Generate a network of four parties and two shards.
//  2. Extract the runtime configuration of the batcher of shard 1 in party 1, and build the access
//     control of the batcher deliver service from the configuration bundle it carries.
//  3. Sign a deliver request with the signing identity of the batcher of shard 1 in party 2 and
//     expect it to be authorized, and do the same with the signing identity of the assembler of
//     party 2.
//  4. Sign a deliver request with the signing identities of the router, the consenter and the batcher
//     of shard 2 of party 2, and expect all three to be refused.
//  5. Extract the runtime configuration of the consenter of party 1, build the access control of the
//     consenter deliver service from the configuration bundle it carries, and expect it to authorize
//     the router of party 2.
func TestExtractedBundleAuthorizesInternalDeliver(t *testing.T) {
	dir := t.TempDir()
	numOfParties := 4
	numOfShards := 2
	configPath := filepath.Join(dir, "config.yaml")
	...
	testLogger := testutil.CreateLoggerForModule(t, "TestExtractedBundleAuthorizesInternalDeliver", zap.DebugLevel)

	batcherOfShard1InParty2 := signerOfNode(t, dir, 2, "batcher1", "batcher1-cert.pem")
	batcherOfShard2InParty2 := signerOfNode(t, dir, 2, "batcher2", "batcher2-cert.pem")
	assemblerOfParty2 := signerOfNode(t, dir, 2, "assembler", "assembler-cert.pem")
	routerOfParty2 := signerOfNode(t, dir, 2, "router", "router-cert.pem")
	consenterOfParty2 := signerOfNode(t, dir, 2, "consenter", "consenter-cert.pem")

	batcherConfigPath := filepath.Join(dir, "config", "party1", "local_config_batcher1.yaml")
	testutil.EditDirectoryInNodeConfigYAML(t, batcherConfigPath, filepath.Join(dir, "storage", "batcher"), "", 0)
	batcherFullConfig, batcherGenesisBlock, err := config.ReadConfig(batcherConfigPath, testLogger)
	require.NoError(t, err)
	batcherConfig := batcherFullConfig.ExtractBatcherConfig(batcherGenesisBlock)

	batcherAccessControl, err := deliver.NewBatcherDeliverVerifier(batcherConfig.Bundle, batcherConfig.ShardId)
	require.NoError(t, err)
	...
	require.ErrorContains(t, err, "batcher of party 2 in shard 2 does not connect to the service of shard 1")
	...
	consenterFullConfig, consenterGenesisBlock, err := config.ReadConfig(consenterConfigPath, testLogger)
	require.NoError(t, err)
	consenterConfig := consenterFullConfig.ExtractConsenterConfig(consenterGenesisBlock)

	consenterAccessControl, err := deliver.NewConsenterDeliverVerifier(consenterConfig.Bundle)
	require.NoError(t, err)
	require.NoError(t, consenterAccessControl.CheckPolicy(signedDeliverRequest(t, routerOfParty2), "arma"))
}

// signerOfNode builds a signer from the signing material of a node of a generated network, taking
// the location of that material from the local configuration of the node.
func signerOfNode(t *testing.T, dir string, partyID int, role string, signCertFile string) *signutil.TestSigner {
	localConfigFile := "local_config_" + role + ".yaml"
	localConfig := testutil.ReadNodeConfigFromYaml(t, filepath.Join(dir, "config", fmt.Sprintf("party%d", partyID), localConfigFile))
	mspDir := localConfig.GeneralConfig.LocalMSPDir
	signer, err := signutil.NewTestSigner(
		filepath.Join(mspDir, "keystore", "priv_sk"),
		filepath.Join(mspDir, "signcerts", signCertFile),
		localConfig.GeneralConfig.LocalMSPID,
	)
	require.NoError(t, err)
	return signer
}

// signedDeliverRequest creates a deliver request signed by the given signer.
func signedDeliverRequest(t *testing.T, signer *signutil.TestSigner) *common.Envelope {
	seekInfo := &protosorderer.SeekInfo{
		Start:    &protosorderer.SeekPosition{Type: &protosorderer.SeekPosition_Newest{Newest: &protosorderer.SeekNewest{}}},
		Stop:     &protosorderer.SeekPosition{Type: &protosorderer.SeekPosition_Specified{Specified: &protosorderer.SeekSpecified{Number: 100}}},
		Behavior: protosorderer.SeekInfo_BLOCK_UNTIL_READY,
	}
	envelope, err := protoutil.CreateSignedEnvelopeWithTLSBinding(common.HeaderType_DELIVER_SEEK_INFO, "arma", signer, seekInfo, int32(0), uint64(0), nil)
	require.NoError(t, err)
	return envelope
}
